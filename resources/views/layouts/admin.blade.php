...
<body class="min-h-screen bg-[#f7f8f4] text-[#17221d]">
    <aside
        class="fixed inset-y-0 left-0 z-10 flex w-[245px] flex-col bg-[#263b32] px-[22px] py-7 text-[#eef3e8] max-md:w-[65px] max-md:px-2">
        <a class="mb-[75px] flex items-center gap-2.5 text-lg font-extrabold tracking-[-.04em] max-md:mb-[55px]"
            href="{{ route('admin.dashboard') }}"><span
                class="grid h-[31px] w-[31px] place-items-center bg-[#d8f06a] text-sm text-[#263b32]">H</span><span
                class="max-md:hidden">Hubla<span class="text-[#8ca097]">/doc</span></span></a>
        <nav class="grid gap-1"><a
                class="p-2.5 text-xs text-[#b3c0b8] hover:bg-white/10 hover:text-white {{ request()->routeIs('admin.dashboard') ? 'bg-white/10 text-white' : '' }}"
                href="{{ route('admin.dashboard') }}">◈ <span class="max-md:hidden">Dashboard</span></a>
            <p class="mt-7 px-2.5 pb-1 font-mono text-[10px] uppercase text-[#8ca097] max-md:hidden">Dokumentasi</p><a
                class="p-2.5 text-xs text-[#b3c0b8] hover:bg-white/10 hover:text-white {{ request()->routeIs('admin.documentations.*') ? 'bg-white/10 text-white' : '' }}"
                ...
                href="{{ route('admin.documentations.create') }}">＋ <span class="max-md:hidden">Tambah
                    dokumentasi</span></a>
            <p class="mt-7 px-2.5 pb-1 font-mono text-[10px] uppercase text-[#8ca097] max-md:hidden">PIC</p><a
                class="p-2.5 text-xs text-[#b3c0b8] hover:bg-white/10 hover:text-white {{ request()->routeIs('admin.persons.*') ? 'bg-white/10 text-white' : '' }}"
        ...
        <form class="mt-auto" method="POST" action="{{ route('admin.logout') }}">@csrf<button
                class="border-0 bg-transparent p-3 text-left text-xs text-[#aab9b0] max-md:text-[0px]" type="submit">↪
